...
struct ActiveWorkoutView: View {
    ...
    let exerciseTypes = ["Squat", "Bench Press", "Deadlift", "Shoulder Press", "Row", "Clean", "Snatch"]
    
    init() {
        let bluetoothManager = BluetoothManager()
        _bluetoothManager = StateObject(wrappedValue: bluetoothManager)
        _sessionManager = StateObject(wrappedValue: WorkoutSessionManager(bluetoothManager: bluetoothManager))
    }
    ...
    var body: some View {
        NavigationStack {
            VStack(spacing: 16) {
                // Status card
                StatusCardView(bluetoothManager: bluetoothManager, sessionManager: sessionManager)
                ...
                if case .workoutActive = sessionManager.currentState, !sessionManager.isRecording {
                    ExerciseSetupView(
                        ...
                        onStartSet: {
                            Task {
                                await sessionManager.startSet(exerciseType: exerciseType, weight: weight)
                            }
                        }
                    )
                }
                ...
                if sessionManager.isRecording, !sessionManager.currentSetData.isEmpty {
            ...
            .alert("Error", isPresented: .constant(sessionManager.errorMessage != nil), actions: {
                Button("OK") {
